...
export const validateDateOfBirth = (value: number | null): TValidateResponse => {
	const now = new Date();
  ...
  const isBirthDateValid = !!birthDate ? birthDate <= now : false;
	if (isBirthDateValid) {
		...
		const message = !!birthDate ? "Дата рождения не может быть в будущем" : "Поле обязательно для заполнения";
